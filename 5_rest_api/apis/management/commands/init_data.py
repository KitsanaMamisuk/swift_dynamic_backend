    ...
    def handle(self, *args, **kwargs):
        try:
            ...
            for _ in range(10):
                ...
                short_name = short_name = fake.unique.pystr_format(string_format='???').upper()
                address = fake.address()
                schools.append(School(name=name, short_name=short_name, address=address))
            ...
            for school in School.objects.all():
                for grade in ["1", "2", "3", "4", "5", "6"]:
                    for room in range(1, 4):
                        classrooms.append(
                            Classroom(grade=grade, room=room, school=school)
                        )
        ...
        try:
            ...
            for school in schools:
                school_teachers = [teacher for teacher in instance_bulk if teacher.school_id == school.id]
                classrooms = school.classrooms.all()
                for classroom in classrooms:
                    selected_teachers = random.sample(school_teachers, min(2, 2))
                ...
                for classroom in classrooms:
                    for _ in range(10):
                        ...
                        if gender == "M":
                            ...
                            last_name = f'{fake.last_name_female()}{fake.city()}'
                        students.append(
                            Student(first_name=first_name, last_name=last_name, gender=gender, classroom=classroom, school=school)
                        )
